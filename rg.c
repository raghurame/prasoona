	...
	for (int i = 0; i < nTimeframes; ++i)
	{
		...
		for (int j = 0; j < nChains; ++j)
		{
			for (int k = 0; k < degreeOfPolymerization; ++k)
			{
				...
				distance = computeDistance (x, y, z, chainCOMs[i][j].x, chainCOMs[i][j].y, chainCOMs[i][j].z);
				radiusOfGyration[i][j] += distance;
			...
			radiusOfGyration[i][j] = pow((radiusOfGyration[i][j]),2);
		}
	}
